...
import { ActionBox, ButtonImageGapGrid, ButtonsBox, CardsBox, MsiteStyledButton } from "./ModalPropItemStyles"
...
export const PropItemsMsiteModal = ({
  ...
  const findPropNestedItems = props?.find((item: any, index: number) => index === selectedIndex)
    ...
    <>
      {findPropNestedItems?.title && (
        <ButtonsBox
          ...
          <MsiteStyledButton>
            {findPropNestedItems?.title !== undefined && typeof findPropNestedItems?.title === "string"
              ? findPropNestedItems?.title
              : isMobile
              ? findPropNestedItems?.title?.mobileTitle || findPropNestedItems?.title?.desktopTitle
      ...
      {props?.map((item: any, index: number) => {
        return (
          <Fragment key={index}>
            {index === selectedIndex &&
              item?.items?.slice(0, countToShowCards)?.map((item: any, index: number) => (
                <ButtonImageGapGrid $paddingTop={55} container columnSpacing={4} key={index}>
                  <Grid
                    item
                    key={index}
                    xl={12}
                    lg={12}
                    md={item?.variant === "ihcl.core.card.social-media-card-image" ? 12 : 6}
                    sm={item?.variant === "ihcl.core.card.social-media-card-image" ? 12 : 6}
                    xs={item?.variant === "ihcl.core.card.social-media-card-image" ? 12 : 6}>
                    <CardsBox
                      onClick={() => {
                      ...
                      {item?.mediaType === "video" && videoPlay
                        ...
                        : (item?.largeImage?.asset?._ref || item?.image?.asset?._ref) && (
                            <Box
                              width={"100%"}
                              height={"100%"}
                              component="img"
                      ...
                      {item?.title && (
                        <Typography variant="m-heading-xs" style={{ margin: "4.69vw 0vw 5.30vw 0vw" }}>
                          {item?.title}
                        </Typography>
                      )}
                    </CardsBox>
      ...
      {findPropNestedItems && findPropNestedItems?.items?.length > countToShowCards && (
