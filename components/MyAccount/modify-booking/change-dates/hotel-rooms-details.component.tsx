...
const HotelRoomsDetails = ({
  ...
  const newRoomData = roomsNewPrice?.filter((room: any) => room?.roomNumber == roomsData?.roomNumber)?.[0]
  ...
  const handleChange = (index: number) => {
    ...
    if (selectedRooms?.[index]) {
      ...
      const newArray = selectedRoomsData?.filter((item: any) => item?.confirmationId !== roomsData?.confirmationId)
      setSelectedRoomsData(newArray)
    } else {
      selectedRooms[index] = true
      const newArray = roomsNewPrice?.filter((item: any) => item?.roomNumber !== roomsData?.roomNumber)
    ...
    const newArray = roomsNewPrice?.map((room: any) => {
      if (room?.roomNumber === newRoomData?.roomNumber) {
        return {
          ...
          priceDifference: newRoomDetails?.total?.amount - getOldPrice(newRoomData?.roomNumber),
        }
      } else {
        return room
      }
      ...
      <MainStack>
        <Stack flexDirection={"row"} columnGap={MobilePxToVw(20)}>
          {roomsData?.roomImgUrl && (
            <RoomImage alt="room-img" component={"img"} src={urlFor(roomsData?.roomImgUrl)?.url()} />
          )}
          {isMobile && (
        ...
        <>
          <Box width="100%">
            <Stack
              ...
              <Box>
                <BoldLabelTypography variant={isMobile ? "m-body-s" : "body-s"}>{data?.package}</BoldLabelTypography>
                <PackageDetailsBox>
                  <Typography variant={isMobile ? "m-body-m" : "body-m"}>{roomsData?.packageName}</Typography>
                  <Typography whiteSpace={"nowrap"} variant={isMobile ? "m-body-l" : "body-l"}>
                    {currency2DecimalSymbol(
                      isComplementary ? complementaryBasePrice || 0 : roomsData?.price,
                      roomsData?.currency,
                    )}
                  </Typography>
                </PackageDetailsBox>
              ...
              <>
                ...
                <Stack sx={{ marginTop: isMobile ? MobilePxToVw(20) : "1.042vw" }}>
                  ...
                  <PackageDetailsBox sx={{ marginTop: "0.521vw" }}>
                    <Typography variant={isMobile ? "m-body-m" : "body-m"}>{data?.priceChange}</Typography>
                    <Typography
                      ...
                      {newRoomData?.priceDifference >= 0
                        ? newRoomData?.priceDifference > 0
                          ? formatCurrencyWithPlus(newRoomData?.priceDifference, newRoomData?.currency)
                          : currency2DecimalSymbol(newRoomData?.priceDifference, newRoomData?.currency)
                        : formatCurrencyWithMinus(newRoomData?.priceDifference, newRoomData?.currency)}
                    </Typography>
                  </PackageDetailsBox>
                </Stack>
              </>
      ...
      {newRoomData && newRoomData?.showRooms && !isComplementary && latestRoomsAvailability?.roomTypes?.length > 0 && (
        <Stack>
          ...
          <Stack flexDirection={"column"} rowGap={isMobile ? MobilePxToVw(20) : DesktopPxToVw(20)}>
            {latestRoomsAvailability?.roomTypes?.[index]?.rooms?.map((newRoom: any, index: number) => (
              <Stack
                ...
                <Stack width={"100%"}>
                  ...
                  <NewSelectedDateComponent
                    ...
                    endDate={newRoomData?.checkOut}
                    newRoom={newRoomData}
                    roomPrice={newRoom?.total?.amount}
                  />
                  <Button
