...
export const GCODetailsContainer = styled(
  Box,
  transientProps
)<{ $minHeight: number; $marginBottom: number; $mobile: boolean }>(
  ({ $minHeight, $marginBottom, $mobile }) => ({
    minHeight: $mobile ? MobilePxToVw($minHeight) : DesktopPxToVw($minHeight),
    marginBottom: $mobile
      ? MobilePxToVw($marginBottom)
      : DesktopPxToVw($marginBottom),
  })
)
...
export const GCODetailsDetailsSpacerContainer = styled(
  ...
  padding: $mobile
    ? `${MobilePxToVw(32)} 0vw ${MobilePxToVw(20)}`
    : `${DesktopPxToVw(27)} 0vw`,
}))
