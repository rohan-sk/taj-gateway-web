...
export const useLeavePageConfirmation = (
  ...
  useEffect(() => {
     ...
     */
    if (shouldPreventLeaving && !isTajLogoClicked && !isMyAccountClicked) {
    ...
    if (shouldPreventLeaving && !isTajLogoClicked && !isMyAccountClicked) {
      SingletonRouter.router.change = async (...args) => {
        ...
        if (
          ...
          changedUrl !== ROUTES?.BOOKING?.CONFIRMED_PAGE
        ) {
          confirmed = await confirmationDialog()
        }

        if (confirmed || changedUrl == ROUTES?.BOOKING?.CONFIRMED_PAGE) {
