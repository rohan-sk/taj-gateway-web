...
const RenderAccountTabs = (props: AccountTabsInterface) => {
  ...
  const personalDetailsVariant =
    props?.variant === "myAccount.tabs.account-and-preferences-tabs"
  ...
  const globalAccountStore = globalContext?.getGlobalStore(
    ...
    if (props?.tabs) {
      if (props?.tabs) {
        for (const item of props?.tabs) {
          if (
            mobileTabsData?.filter((data: any) => data?.title == item?.title)
              ?.length === 0
          ) {
  ...
  const renderMobileView = () => {
    ...
    const activeTab: any = mobileTabsData?.find(
      (item: any) => item?.title === currentTab?.value
    )
      ...
      <Box
        ...
        <TabListBox>
            ...
            <RowStack key={index}>
              <TabTypography
                ...
                onClick={() => {
                  index > 1
                    ? handleModelOpening()
                    : handleChange(index, item.title, item)
                }}>
          ...
          <RowStack onClick={handleModelOpening}>
            {FinalTabsList.length <= 2 && (
              <MoreLabelTypography variant="m-body-m">
                {CONSTANTS?.MORE}
              </MoreLabelTypography>
            )}
            <StyledDownArrow />
        ...
        {openModel && (
          <BasicModal
            ...
            mobileTop={`${
              isMobile ? MobilePxToVw(75) : DesktopPxToVw(75)
            } !important`}
        ...
        {props?.tabs ? (
          <>
            ...
            <Grid
              ...
              sx={{ mb: isMobile ? "0vw" : DesktopPxToVw(10) }}>
