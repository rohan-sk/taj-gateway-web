  ...
  return (
    <Box p="6vw 12.5vw">
      ...
      <Grid container spacing={3.5}>
        {datainfo?.info
          ?.slice(0, datainfo?.info?.length >= 6 ? (showMore ? datainfo?.info?.length : 6) : datainfo?.info?.length)
          ?.map((listdata: any, index: number) => (
            <Grid item xs={6} md={4} lg={4} xl={4} key={index}>
              <Typography variant="heading-xs" sx={{ paddingBottom: "10px" }}>
                {listdata?.cityName}
              </Typography>
              <Typography sx={{ color: "#45443F", paddingBottom: "0px" }}>{listdata?.address}</Typography>
              <Typography
                sx={{
              ...
              <Typography sx={{ color: "#45443F", paddingBottom: "0px" }}>{listdata?.phone}</Typography>
              <Typography sx={{ color: "#45443F" }}>{listdata?.fax}</Typography>
              <Typography variant="heading-xs" sx={{ paddingTop: "14px", fontWeight: 700 }}>
                {listdata?.mail}
              </Typography>
        ...
        {datainfo?.info?.length > 6 && !showMore && (
          <Box
            ...
            <Typography sx={{ color: " #AD8B3A", textAlign: "center" }}>LOAD MORE</Typography>
